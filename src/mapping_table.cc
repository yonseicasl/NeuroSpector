...
void mapping_table_t::print_stats() {
    ...
    handler.print_line(63, "-");
    std::cout << "# TOTAL MACs                : " << mac_cnts << std::endl;
...
void mapping_table_t::update_tile_size() {
    mac_cnts = layer_vals.at(0) * layer_vals.at(1) * layer_vals.at(2) 
             * layer_vals.at(3) * layer_vals.at(4) * layer_vals.at(5) * layer_vals.at(6);
    // MAC
...
void mapping_table_t::update_energy_stats() {
    ...
    if(L2_dataflow == dataflow_t::IS) {
        energy_stats.DRAM_total_energy = between_DRAM_L2_is * energy_stats.DRAM_read_write_avg; 
        energy_stats.L2_total_energy += between_DRAM_L2_is * energy_stats.L2_read_write_avg;
    }
    else if(L2_dataflow == dataflow_t::WS) {
        energy_stats.DRAM_total_energy = between_DRAM_L2_ws * energy_stats.DRAM_read_write_avg; 
        energy_stats.L2_total_energy += between_DRAM_L2_ws * energy_stats.L2_read_write_avg;

    }
    else if(L2_dataflow == dataflow_t::OS) {
        energy_stats.DRAM_total_energy = between_DRAM_L2_os * energy_stats.DRAM_read_write_avg; 
        energy_stats.L2_total_energy += between_DRAM_L2_os * energy_stats.L2_read_write_avg;
    }
    energy_stats.total();
}
...
void mapping_table_t::print_energy_stats() {
    std::cout << "# ENERGY STATS" << std::endl;
    std::cout << std::setw(12) << "COMPONENT  |" 
              << std::setw(10)  << "DATAFLOW"
              << std::setw(10) << "ENERGY" << std::endl; 
    handler.print_line(43, "-");
    std::cout << std::setw(12) << "  MAC  |" 
              << std::setw(10)  << static_cast<unsigned>(L0_dataflow)
              << std::setw(15) << std::setprecision(10) << energy_stats.MAC_total_energy << std::endl; 
    std::cout << std::setw(12) << "  L1   |" 
    ...
    std::cout << std::setw(12) << " DRAM  |" 
              << std::setw(10)  << "NONE"
              << std::setw(15) << std::setprecision(10) << energy_stats.DRAM_total_energy << std::endl; 
    handler.print_line(43, "-");
