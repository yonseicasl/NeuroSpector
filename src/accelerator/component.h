...
public:
    ...
    virtual std::vector<unsigned> get_allocated_size(direction_type_t direction_) const;     // Get allocated data size
    virtual std::vector<unsigned> get_tile_access_count(operation_type_t operation_, 
                                                        direction_type_t direction) const;  // Get tile access count
    virtual std::vector<unsigned> get_active_components() const;  // Get active components
    virtual std::vector<data_type_t> get_bypass() const;          // Get bypass 
...
protected:
    ...
    std::vector<data_type_t> bypass;             // Bypassed data types
    std::map<direction_type_t, std::vector<unsigned>> allocated_size;        // Allocated size
    std::vector<unsigned> active_components;     // Num. active components 
    // std::map<operation_type_t, std::vector<unsigned>> tile_access_count;        // Tile access count
    std::map<operation_type_t, std::vector<unsigned>> access_count_to_upper;        // Tile access count
    std::map<operation_type_t, std::vector<unsigned>> access_count_to_lower;        // Tile access count
